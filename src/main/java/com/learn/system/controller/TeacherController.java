...
public class TeacherController {
    ...
    public ModelAndView insertClass(@RequestParam("classNo")String classNo,
        ...
        //先检查是否存在相同的班级号
        int isExist = teacherService.isExistClassNo(classNo);
        ...
        if(isExist == 1){           //存在，不能插入
            ...
            teacherService.insertClassInfo(cla);
    ...
    public String toInsertStuInfo(Model model, HttpSession session){
        ...
        str = teacherService.queryAllClassNo();      //获取所有的班级号
    ...
    public ModelAndView insertStu(@RequestParam("stuNo") String stuNo,
        ...
        int isExist = teacherService.isExistStuNo(stuNo);
        if(isExist != 0){   //存在
            mv.addObject("isExist", 1);
        } else {
            //将生日字符串转换成date类型
            ...
            teacherService.insertStudentInfo(stu);
            mv.addObject("success", 1);
        }
    ...
    public ModelAndView deleteStu(@PathVariable(value="pageNum")int pageNum,
        ...
        int total = teacherService.queryAllStudent().size(); //学生信息总数
        ...
        studentList = teacherService.querySomeStudent(teacherService.queryAllStudent(),
                pageNum, offset);
    ...
    public ModelAndView delete(@PathVariable("stuNo")String stuNo,
                               ModelAndView mv, HttpSession session){
        teacherService.deleteStuByNo(stuNo);
